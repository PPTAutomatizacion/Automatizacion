...
$(function () {
    ...
    ko.bindingHandlers.slideVisible = {
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            let show = typeof (valueUnwrapped) === 'boolean' ? valueUnwrapped : (ko.unwrap(valueUnwrapped.visible) || false);
            if (show === true) {
                $(element).show();
            } else {
                $(element).hide();
            ...
            $(element).data("slideVisibleState", show);
        },
        update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            var value = valueAccessor();
            var valueUnwrapped = ko.unwrap(value);
            let direction = typeof (valueUnwrapped) === 'boolean' ? 'upDown' : (ko.unwrap(valueUnwrapped.direction) || 'upDown');
            let duration = typeof (valueUnwrapped) === 'boolean' ? (direction === 'upDown' ? 200 : 400) : (ko.unwrap(valueUnwrapped.duration) || (direction === 'upDown' ? 200 : 400));
            let show = typeof (valueUnwrapped) === 'boolean' ? valueUnwrapped : (ko.unwrap(valueUnwrapped.visible) || false);

            if (show === $(element).data("slideVisibleState"))
                return;
        ...
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            $(document).ajaxError((e, xhr) => {
                ...
                if (xhr.statusText === 'abort') {
                    console.warn("ajax request aborted", e, xhr);
                    return;
                }
    ...
    ko.bindingHandlers.autocomplete = {
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            if (options.serviceUrl) {
                options.serviceUrl = $("body").data("baseurl") + ko.unwrap(options.serviceUrl);
            }

            $(element).devbridgeAutocomplete(options);
        }
    };
    ...
    ko.bindingHandlers.hoverVisible = {
        ...
        init: function (element, valueAccessor) {
            let value = valueAccessor();
            let targetSelector = typeof value === 'string' ? value : ko.unwrap(value.targetSelector);
            let effect = typeof value === 'string' ? ko.bindingHandlers.hoverVisible.effects.toggle : ko.unwrap(value.effect);

            function showOrHideElement(show) {
                if (effect === ko.bindingHandlers.hoverVisible.effects.toggle)
                    $(targetSelector).toggle(show);
                else if (effect === ko.bindingHandlers.hoverVisible.effects.fade)
                    show ? $(targetSelector).stop().fadeIn() : $(targetSelector).stop().fadeOut();
                else
                    console.error("hoverVisible: effect not found or is invalid (" + effect + ").");
            }

            let hideElement = showOrHideElement.bind(null, false);
            let showElement = showOrHideElement.bind(null, true);

            //touch events
            if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                ko.utils.registerEventHandler($(element), "click", () => {
                    $(targetSelector).is(':visible') ? hideElement() : showElement();
                });
            }
            else {
                ko.utils.registerEventHandler($(element), "mouseover", showElement);
                ko.utils.registerEventHandler($(element), "mouseout", hideElement);
            }

            setTimeout(() => $(targetSelector).toggle(false), 100);
        }
    };

    ko.bindingHandlers.tilesGrid = {
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            let defaults = {
                width: 12,
                cellHeight: 80,
            ...
            //configure tiles elements
            let configTileItem = (tileItem) => {
                let grid = $(tileItem).parent('.grid-stack').data('gridstack');
                ko.utils.domNodeDisposal.addDisposeCallback(tileItem, function () {
                    grid.removeWidget(tileItem);
                });
                grid.addWidget(tileItem);
            };

            $(element).gridstack(options);

            viewModel.afterAddTile = (elements) => {
                let item = _.find(elements, function (i) { return i.nodeType == 1 });
                configTileItem(item);
            }

            let currentTilesElements = $(element).find('.grid-stack-item');
            _.forEach(currentTilesElements, function (tileElement) {
                configTileItem(tileElement);
            });

            $(document).one("app.initialized", function () {
                $(element).on('change', (evt, items) => {
                    _.forEach(items, (item) => {
                        let tile = ko.dataFor(item.el[0]);
                        if (tile) {
                            if (tile.X)
                                tile.X(item.x);
                            if (tile.Y)
